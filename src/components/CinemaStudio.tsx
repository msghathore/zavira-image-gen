...
interface DownloadButtonProps {
  imageUrl: string;
  imageId: string;
  compact?: boolean; // For smaller gallery items
}

function DownloadButton({ imageUrl, imageId, compact = false }: DownloadButtonProps) {
  ...
  const downloadImage = async (format: 'png' | 'webp' | 'jpeg') => {
    ...
    try {
      ...
      canvas.toBlob((blob) => {
        ...
        a.href = url;
        a.download = `image-${imageId || Date.now()}.${extension}`;
  ...
  const formats = [
    { id: 'png', label: 'PNG', desc: 'Original (Lossless)', icon: '🎨' },
    { id: 'webp', label: 'WebP', desc: 'Smaller (Lossless)', icon: '🌐' },
    { id: 'jpeg', label: 'JPEG', desc: 'Smallest (High Quality)', icon: '📷' },
  ] as const;
    ...
    return (
      <div ref={dropdownRef} className="relative">
        <button
          ...
          className="p-1.5 bg-black/70 hover:bg-lime-500 hover:text-black rounded-md transition-colors"
        ...
        {showDropdown && (
          <div className="absolute top-full right-0 mt-1 bg-zinc-900 border border-zinc-700 rounded-lg shadow-xl z-50 min-w-[140px] overflow-hidden">
            {formats.map((fmt) => (
              <button
                key={fmt.id}
                onClick={(e) => {
                  e.stopPropagation();
                  downloadImage(fmt.id);
                }}
                className="w-full px-3 py-2 text-left text-xs hover:bg-lime-500 hover:text-black transition-colors flex items-center gap-2"
              >
                <span>{fmt.icon}</span>
                <div>
  ...
  return (
    <div ref={dropdownRef} className="relative">
      <button
        ...
        className="px-2 py-1 bg-black/70 hover:bg-lime-500 hover:text-black rounded-md text-xs flex items-center gap-1 transition-colors"
        title="Download image"
      >
        {isConverting ? (
          <div className="w-3.5 h-3.5 border-2 border-current border-t-transparent rounded-full animate-spin" />
      ...
      {showDropdown && (
        <div className="absolute top-full right-0 mt-1 bg-zinc-900 border border-zinc-700 rounded-lg shadow-xl z-50 min-w-[160px] overflow-hidden">
          {formats.map((fmt) => (
            <button
              key={fmt.id}
              onClick={(e) => {
    ...
    >
      ...
      {content.type === 'image' && loadedUrl && !isLoading && !error && (
        <div className="absolute top-1 right-1 flex gap-1 opacity-0 group-hover:opacity-100 transition-all">
          {/* Download button with format selector */}
          <DownloadButton imageUrl={loadedUrl} imageId={content.id} compact />
            ...
            <div className="relative max-w-md max-h-96 rounded-lg overflow-hidden group">
              ...
              {imageLoaded && (
                <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-all">
                  {/* Download button with format selector */}
                  <DownloadButton imageUrl={displayUrl} imageId={content.id} />
                  {/* Set as Reference button */}
                  {onSetReference && (
                    <button
                      onClick={(e) => {
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-14 bg-zinc-900/50 border-r border-zinc-800 flex flex-col items-center py-4 gap-3">
          ...
          <button
            ...
            className={`w-10 h-10 rounded-lg flex items-center justify-center transition-colors ${
              showElements ? 'bg-lime-500 text-black' : 'bg-zinc-800 hover:bg-zinc-700'
            }`}
            title="Elements"
          >
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
